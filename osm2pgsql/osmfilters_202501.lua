...
local filteredlist = { "amenity", "shop", "tourism"}
-- Amenity
local amenityUndescoreList = { "vending_machine", "fast_food",
                               "grit_bin", "ice_cream", "bicycle_rental", "arts_centre",
                               "car_rental", "car_sharing", "bureau_de_change",
                               "internet_cafe", "car_wash"}

local amenitySubclassList = { "restaurant", "cafe", "bar", "bank", "pharmacy", "pub", "dentist", "theater",
                              "kindergarten", "nightclub", "cinema", "library", "biergarten",
                              "gambling", "marketplace", "casino", "stripclub", "fraternity", "tailor" }
-- Shop
local shopUndescoreList = { "travel_agency", "department_store", "hearing_aids", "dry_cleaning",
...
local tourismSubclassList = { "hotel", "hostel", "casino" }
...
local function refineclasses(list1, k ,v)
    ...
    if not contains(list1, k) then
        return "do_nothing"
        -- Superclasses
    elseif (starts_with(k, "amenity") or starts_with(k, "shop"))
            and starts_with(v, "yes")
    then return k_upper
    elseif (starts_with(k, "tourism")) and starts_with(v, "yes")
